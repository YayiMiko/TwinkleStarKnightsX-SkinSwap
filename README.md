...
本 MOD 会将《闪耀星骑士》中角色发动“通常攻击 1”和“通常攻击 2”时出现的角色演出替换为该角色的高画质成人变身动画（R18）。当前提供 PC 正式版和 Android 正式版。它不会改变实际攻击动作、伤害或战斗速度。
...
### Android 版 Release 0.3.1

[下载 Android 版 Release 0.3.1](https://github.com/YayiMiko/TSKSkinSwap/releases/tag/android-v0.3.1)

Android 版会自动查询友仓最新的兼容安装包，保留汉化和手机上的大型游戏数据，并只补齐缺失的变身资源。0.3.1 增加了醒目的中文手机操作提示，修复中文 Windows 路径导致安装失败及安装后文件夹被占用的问题；同时包含 0.3.0 的动画稳定性修复。使用前请阅读 [Android 版说明](README.android.md)。
...
Android 首次请求某个角色的资源时，如果加载尚未完成，本次攻击会保留原演出。等待片刻或先播放一次该角色的原生变身动画，再次播放通常攻击即可。
