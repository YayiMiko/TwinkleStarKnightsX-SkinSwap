...
本 MOD 会将《闪耀星骑士》中角色发动“通常攻击 2”时出现的角色演出替换为该角色的高画质成人变身动画（R18）。“通常攻击 1”保留游戏原始演出。它不会改变实际攻击动作、伤害或战斗速度。

PC 1.3.0 和 Android 0.4.0 均只替换“通常攻击 2”。首次安装或游戏更新后，个别角色的资源尚未加载时，可先到图鉴播放一次该角色的相关通常攻击动画，等待加载完成后再返回重试。
...
[下载 PC 版 Release 1.3.0](https://github.com/YayiMiko/TSKSkinSwap/releases/tag/v1.3.0)

1. 下载 `TskSkinSwap-PC-v1.3.0.zip` 并完整解压。
...
已经安装 PC 旧版的用户无需卸载：下载并完整解压 1.3.0，用新的 `TskSkinSwap` 文件夹覆盖原文件夹，然后在游戏关闭时再次运行 `Apply-TskSkinSwap.bat` 即可。已经下载且仍然有效的资源会继续复用。

### Android 版 Release 0.4.0

[下载 Android 版 Release 0.4.0](https://github.com/YayiMiko/TSKSkinSwap/releases/tag/v1.3.0)

Android 版会自动查询友仓最新的兼容安装包，保留汉化和手机上的大型游戏数据，并只补齐缺失的变身资源。使用前请阅读 [Android 版说明](README.android.md)。
...
确认测试角色拥有成人变身动画，并实际触发“通常攻击 2”。首次安装或游戏更新后如暂未替换，请先到图鉴播放一次该角色的相关通常攻击动画，等加载完成后再试。游戏更新后请重新应用 MOD。

Android 首次请求某个角色的资源时，如果加载尚未完成，本次通常攻击 2 会保留原演出。先到图鉴播放一次该角色的相关通常攻击动画，等待加载完成后再试即可。
